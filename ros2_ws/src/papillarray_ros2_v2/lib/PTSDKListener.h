...
private:
	...
	char* logBuf;				// Buffer for the log file (length: LOG_BUF_LEN)

	int nReadBuf;
	BYTE *readBuf;					// (length: BIGARRAY_LEN)
	...
	void printCommsErrorMessage(const char* msg);
	...
	int sprintf_s(char* _Buffer, std::size_t _BufferSize, const char* format, ...);
...
public:
	...
	//bool readAllInBufferAndParse(IN const bool isFlush);
