...
int main(int argc, char *argv[])
{
    ...
    int ret = listener.connectAndStartListening(
        options.port.c_str(), options.baud_rate, parity, byte_size, options.is_flush);
    if (ret != 0) {
    ...
    while (g_running) {
        ...
        // 获取当前最新传感器时间戳
        unsigned long ts_us = static_cast<unsigned long>(
            sensors[options.sensor_index].getTimestamp_us());
